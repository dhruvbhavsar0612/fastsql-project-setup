...
def make_choices(enum_class: Any, labels: dict[str, str] | None = None) -> list[dict]:
    ...
    for item in enum_class:
        label = labels.get(item.value, item.value.replace("_", " ").title()) if labels else item.value.replace("_", " ").title()
...
async def gather_project_basics() -> dict:
    ...
    python_version = await questionary.select(
        "Python version:",
        choices=make_choices(PythonVersion, {
            "3.10": "Python 3.10",
            "3.11": "Python 3.11",
...
async def gather_auth_config() -> dict:
    ...
    auth_method = await questionary.select(
        ...
        choices=make_choices(AuthMethod, {
...
async def gather_messaging_config() -> dict:
    ...
    message_broker = await questionary.select(
        "Message broker:",
        choices=make_choices(MessageBroker, {
            "rabbitmq": "RabbitMQ",
            "redis": "Redis",
            "none": "None",
        }),
    ...
    task_queue = await questionary.select(
        "Task queue:",
        choices=make_choices(TaskQueue, {
...
async def gather_cache_config() -> dict:
    ...
    cache_backend = await questionary.select(
        "Cache backend:",
        choices=make_choices(CacheBackend, {
...
async def gather_logging_config() -> dict:
    ...
    logging_lib = await questionary.select(
        "Logging library:",
        choices=make_choices(LoggingLib, {
...
async def gather_dev_tools() -> dict:
    ...
    package_manager = await questionary.select(
        "Package manager:",
        choices=make_choices(PackageManager, {
    ...
    type_checker = await questionary.select(
        "Type checking:",
        choices=make_choices(TypeChecker, {
...
async def gather_deployment_config() -> dict:
    ...
    github_workflow = await questionary.select(
        ...
        choices=make_choices(GitHubWorkflow, {
...
async def gather_structure_config() -> dict:
    ...
    project_structure = await questionary.select(
        "Project structure:",
        choices=make_choices(ProjectStructure, {
